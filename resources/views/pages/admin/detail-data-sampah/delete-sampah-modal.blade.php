    ...
    <div class="flex items-center justify-center min-h-screen px-4 text-center sm:block sm:p-0 relative z-50">
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                Apakah Anda yakin ingin menghapus data sampah <span class="font-medium text-gray-700 dark:text-gray-300" x-text="selectedWaste.name"></span>? Tindakan ini tidak dapat dibatalkan.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-800 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form x-bind:action="'{{ route('admin.waste-types.destroy', '') }}/' + selectedWaste.id" method="POST" class="sm:flex sm:flex-row-reverse">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
