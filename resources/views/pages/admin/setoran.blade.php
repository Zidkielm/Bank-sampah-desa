<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto" x-data="{
        ...
        async getWasteTypePrice(index) {
            ...
            try {
                const response = await fetch(`{{ route('waste-type-price', '') }}/${this.wasteItems[index].waste_type_id}`);
        ...
        async getDepositDetails(id) {
            try {
                const response = await fetch(`{{ route('deposit.show', '') }}/${id}`, {
                    headers: {
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            <form action="{{ route('deposit.report') }}" method="POST" target="_blank">
                @csrf
        ...
        @include('pages.admin.detail-setoran.detail-modal')
        @include('pages.admin.detail-setoran.add-modal')
    </div>
</x-app-layout>
