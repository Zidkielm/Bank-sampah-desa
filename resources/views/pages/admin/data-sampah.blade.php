<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto" x-data="{
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            <div class="mb-6">
                <form action="{{ route('data-sampah') }}" method="GET">
                    <div class="relative">
                        ...
                        <input type="text" name="search"
                            ...
                            placeholder="Masukkan nama sampah" value="{{ request('search') }}">
                    </div>
                </form>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @forelse($wasteTypes as $index => $waste)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-4 py-4 text-gray-800 dark:text-gray-300">
                                    {{ 'Rp ' . number_format($waste->price_per_kg, 0, ',', '.') }}</td>
                                <td class="px-4 py-4">
                                    @if ($waste->image_path)
                                        <img src="{{ asset('storage/' . $waste->image_path) }}"
                                            alt="{{ $waste->name }}" class="w-16 h-16 object-cover rounded">
                                    @else
                                        <div
