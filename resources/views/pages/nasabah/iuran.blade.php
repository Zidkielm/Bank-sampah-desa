<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto" x-data="{
        ...
        async getFeeDetails(id) {
            try {
                const response = await fetch(`{{ route('nasabah.monthly-fee.show', '') }}/${id}`, {
                    headers: {
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
            <form action="{{ route('nasabah.iuran') }}" method="GET">
        ...
        @include('pages.nasabah.detail-iuran.detail-modal')
        @include('pages.nasabah.detail-iuran.add-modal')
    </div>
</x-app-layout>
